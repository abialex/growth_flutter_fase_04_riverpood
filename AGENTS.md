...
Every component fits one of nine categories: colors, typography, spacing, buttons, text fields, cards, chips, loaders, banners/empty states. A new component must fit one of these, not invent a tenth. The one known gap left in the existing set: `AppButton` has no demo of its `focus` state or a trailing-icon example in `example/`.
...
flutter test      # run tests, if/when a test/ directory exists
...
`example/pubspec.yaml` depends on the root package via `app_ui_kit: path: ../`, so any change to `lib/` is picked up immediately without republishing — no bootstrap step needed.

The root package lints with `very_good_analysis` (`example/` still uses plain `flutter_lints`), which is considerably stricter — most notably `public_member_api_docs` (every public member needs a `///` doc comment). Three files opt out of that one rule via a documented `// ignore_for_file:`/`// ignore:` because their members are self-explanatory by name (`color_primitives.dart`, `color_roles.dart`) or because the flagged value is intentionally explicit despite matching a Flutter default (`app_card.dart`). Follow that precedent — a real reason inline, not a bare ignore — rather than adding undocumented ignores.
...
- `tokens/` — the only source of visual values.
  ...
  - `enums/` (e.g. `app_brand.dart`, `app_emphasis.dart`) each live in their own file — this pattern repeats per layer (`atoms/enums/`, `molecules/enums/`, `organisms/enums/`).
...
Each layer has a barrel file (`atoms.dart`, `molecules.dart`, `organisms.dart`, `tokens.dart`) re-exported from `lib/app_ui_kit.dart`, the package's single public entry point. A new component must be added to its layer's barrel and given a demo page in `example/lib/pages/`, or it isn't considered done.
...
`google_fonts` is the only external dependency of the root package and is optional/deferred to `typography_tokens.dart`; do not add other dependencies without checking whether Flutter/Dart's SDK already solves the problem.
