...
class AppChip extends StatelessWidget {
  /// Creates a chip. [label] is required; everything else has a default — see
  ...
  /// Chip type: `assist` (default), `filter` or `input`. See the class doc for
  /// what each one does.
  ...
  /// Disables the chip, greying it out and preventing taps.
  final bool isDisabled;

  /// For `filter` chips, whether the chip is selected. See the class doc for
  /// how to use this with [onSelected].
  final bool isSelected;

  /// Called when the chip is tapped. Only used for `assist` chips.
  final VoidCallback? onTap;

  /// Called when a `filter` chip is tapped. Only used for `filter` chips.
  final ValueChanged<bool>? onSelected;

  /// Called when the delete icon is tapped. Only used for `input` chips.
